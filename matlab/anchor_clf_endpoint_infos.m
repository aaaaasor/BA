...
endpoint_early_gain = endpoint_gain_pair(constraint_cfg, ...
	'anchor_clf_endpoint_phi1_early_gain', '', ...
	'anchor_clf_phi1_early_gain', '', 0.0);
endpoint_phi1_max = endpoint_gain_pair(constraint_cfg, ...
	'anchor_clf_endpoint_phi1_max', '', ...
...
for endpoint_idx = 1:2
	...
	endpoint_cfg.anchor_clf_phi1_early_gain = ...
		endpoint_early_gain(endpoint_idx);
	endpoint_cfg.anchor_clf_phi1_max = endpoint_phi1_max(endpoint_idx);
	if ~is_safeflow
...
if isempty(pair) && ~isempty(endpoint_legacy_field)
	pair = struct_field_default(constraint_cfg, endpoint_legacy_field, []);
end
if isempty(pair)
	if isempty(global_legacy_field)
