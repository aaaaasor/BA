...
% 单发(非顺序)路径下的障碍平滑目标结果，由下面的 else 分支填写。
direct_smoothness = struct('enabled', false, 'reference_cost', 0, ...
	'predicted_cost', 0);
direct_smoothness_contribution = zeros(n_u, 1);
if sequential_increment_qp
	...
	[A_qp, b_qp, constraint_types] = active_qp_constraints(grad_x, ...
		...
		obstacle_info, grad_x_active, boundary_info, terminal_info);
	has_control_equalities = ~isempty(struct_field_default( ...
		constraint_cfg, 'endpoint_hold_velocity_matrix', []));
	% 障碍平滑目标(单发路径)。这里没有前置阶段，所以线性化参考点是 u=0，
	% 且全部控制列都可用。第三层走顺序路径，不经过这里。
	direct_smoothness = third_obstacle_smoothness_cost( ...
		stats, constraint_cfg, t, zeros(n_u, 1), 1:n_u);
	if ~isempty(A_qp) || has_control_equalities || direct_smoothness.enabled
		if direct_smoothness.enabled
			[u_col, qp_exitflag, qp_slack, qp_residuals, qp_iterations, ...
				qp_solve_seconds, qp_u_row_contributions, ...
				qp_u_equality_contribution, direct_smoothness_contribution] = ...
				solve_control_least_squares_qp( ...
				A_qp, b_qp, constraint_types, constraint_cfg, t, stats, ...
				terminal_info, integral_residual_without_u, ...
				terminal_residual_without_u, ...
				direct_smoothness.G, direct_smoothness.e);
			direct_smoothness.reference_cost = ...
				0.5 * sum(direct_smoothness.e .^ 2);
			direct_smoothness.predicted_cost = 0.5 * sum( ...
				(direct_smoothness.G * u_col + direct_smoothness.e) .^ 2);
		else
			[u_col, qp_exitflag, qp_slack, qp_residuals, qp_iterations, ...
				qp_solve_seconds, qp_u_row_contributions, ...
...
u_contribution_smoothness = zeros(n_u,1);
smoothness_reference_cost = 0;
smoothness_predicted_cost = 0;
if sequential_increment_qp && isfield(sequential_result,'smoothness') && ...
        sequential_result.smoothness.enabled
    u_contribution_smoothness = sequential_result.smoothness_objective_contribution;
    smoothness_reference_cost = sequential_result.smoothness.reference_cost;
    smoothness_predicted_cost = sequential_result.smoothness.predicted_cost;
elseif ~sequential_increment_qp && direct_smoothness.enabled
    u_contribution_smoothness = direct_smoothness_contribution;
    smoothness_reference_cost = direct_smoothness.reference_cost;
    smoothness_predicted_cost = direct_smoothness.predicted_cost;
end
kkt_u_reconstruction_error = norm(u_col - sum( ...
	qp_u_row_contributions, 1)' - qp_u_equality_contribution - ...
    u_contribution_smoothness);
if any(hocbf_rows)
...
% The affine objective center is separate from constraint KKT contributions.
diagnostics.u_contribution_smoothness = u_contribution_smoothness;
diagnostics.smoothness_reference_cost = smoothness_reference_cost;
diagnostics.smoothness_predicted_cost = smoothness_predicted_cost;
