...
track_point_distance = nan(n_points, n_trajectories);
obstacle_point_distance = nan(n_points, n_trajectories);
...
for trajectory_idx = 1:n_trajectories
	...
	for point_idx = 1:n_points
		...
		% The global rail fields are signed Euclidean distances already.
		track_point_distance(point_idx, trajectory_idx) = ...
			track_point_h(point_idx, trajectory_idx);
		obstacle_h = inf;
		obstacle_distance = inf;
		for obstacle_idx = 1:size(obstacles.centers, 2)
			[h_now, grad_now] = obstacle_level_and_gradient( ...
				p, obstacles, obstacle_idx);
			obstacle_h = min(obstacle_h, h_now);
			% Superellipse h is dimensionless.  h/|grad h| is its local signed
			% physical-normal distance, matching the normalization used by the
			% third-level joint-safety soft minimum.
			obstacle_distance = min(obstacle_distance, ...
				h_now / max(norm(grad_now), 1e-12));
		end
		obstacle_point_h(point_idx, trajectory_idx) = obstacle_h;
		obstacle_point_distance(point_idx, trajectory_idx) = obstacle_distance;
		point_safe_mask(point_idx, trajectory_idx) = ...
			track_point_distance(point_idx, trajectory_idx) >= -tolerance && ...
			obstacle_distance >= -tolerance;
	end
...
% Segment-junction discontinuity, reported separately from CS/AS.  CS is a
% curvature metric on the merged 65-point reconstruction so it stays
% comparable with single-shot baselines; the cost of generating in segments
% belongs in its own number rather than folded into that curvature.
junction_gap_values = smoothness_details.junction_gaps;
safe_flow_metrics.junction_gap_median = ...
	median(junction_gap_values(:), 'omitnan');
safe_flow_metrics.junction_gap_max = max(junction_gap_values(:), [], 'omitnan');
...
safety_details = struct( ...
	...
	'track_point_distance', track_point_distance, ...
	'obstacle_point_distance', obstacle_point_distance, ...
...
evaluation_metadata = struct( ...
	...
	'safety_tolerance_mode', 'third-level physical normal distance', ...
...
fprintf(['  Junction gap (segment reconstruction): median %.4g, ', ...
	'max %.4g  [CS/AS are computed on the merged %d-point trajectory]\n'], ...
	safe_flow_metrics.junction_gap_median, ...
	safe_flow_metrics.junction_gap_max, ...
	round(median(smoothness_details.points_per_trajectory)));
