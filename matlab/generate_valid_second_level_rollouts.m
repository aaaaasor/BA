...
function [times, accepted_path, diagnostics, accepted_x_init, ...
	accepted_seeds, acceptance, accepted_uncertainty] = ...
...
while ~isempty(pending_queue)
	...
	if any(attempt_counts(batch_parents) > max_attempts)
		failed = batch_parents(attempt_counts(batch_parents) > max_attempts);
		if segment_retry
			failed_pairs = [floor((failed-1)/full_segment_count)+1, ...
				mod(failed-1,full_segment_count)+1];
	...
	if line_filter_enabled && isempty(pending_queue) && full_segment_count > 1
...
acceptance = struct( ...
	'enabled', true, ...
	'criterion', ['round batches; each full trajectory passes hard variance ', ...
		'filters and the final discrete track-point check'], ...
	'tolerance', tolerance, ...
	'base_seed', base_seed, ...
...
if segment_retry
	...
	acceptance.accepted_count = full_parent_count;
	acceptance.accepted_segment_count = n_parents;
	...
	fprintf('Segment-local filter complete: %d/%d full trajectories, %d accepted segments, %d segment attempts.\n', ...
		full_parent_count,full_parent_count,n_parents,sum(attempt_counts));
else
...
terminal_list = cellfun(@(d) d.terminal_safety_filter, diag_list, ...
	'UniformOutput', false);
merged.terminal_safety_filter = struct( ...
	'corrected_samples', sum(cellfun(@(d) d.corrected_samples, terminal_list)), ...
	'total_samples', sum(cellfun(@(d) d.total_samples, terminal_list)), ...
	'max_initial_violation', max(cellfun( ...
	@(d) d.max_initial_violation, terminal_list)), ...
	'max_final_violation', max(cellfun( ...
	@(d) d.max_final_violation, terminal_list)), ...
	'max_state_correction_norm', max(cellfun( ...
	@(d) d.max_state_correction_norm, terminal_list)));
end
