...
% trajectory only when a hard variance/HOCBF condition or a final discrete
% geometry point check fails, then retry the rejected trajectory index with
% its next reproducible seed until the requested accepted count is available.
% Unlike second-level segments, all five first-level points are generated by
% that seed, so the final geometry checks inspect every point.
cfg.first_level_seed_filter.enabled = true;
cfg.first_level_seed_filter.implementation_version = 2;
...
cfg.first_level_seed_filter.final_track_point_filter_enabled = true;
% First-level anchors must retain the controller's inward track margin.
% Other levels keep raw-boundary acceptance (the sampler default).
...
cfg.first_level_seed_filter.final_obstacle_point_filter_enabled = true;
...
cfg.first_level_seed_filter.parallel_trajectory_batch_size = ...
    cfg.first_level_seed_filter.target_trajectory_count;
% Each second-level segment has its own seed. If any segment fails, advance
% every segment seed and rerun the whole parent; keep other accepted parents.
cfg.second_level_seed_filter.enabled = 1;
cfg.second_level_seed_filter.seed_scope = 'segment';
cfg.second_level_seed_filter.retry_scope = 'trajectory';
% v11: independent segment seeds, with whole-trajectory rejection/retry.
% Final geometry checks test discrete points, not continuous chords.
% Apply the same final discrete-point test to every enabled obstacle.  Only
% test each segment's generated interior points 2/3/4: endpoints 1/5 are
% snapped to fixed parent anchors and cannot be changed by retrying a seed.
cfg.second_level_seed_filter.implementation_version = 11;
cfg.second_level_seed_filter.base_seed = cfg.second_level_rollout_seed;
cfg.second_level_seed_filter.target_trajectory_count = ...
    cfg.first_level_generation_samples;
cfg.second_level_seed_filter.max_attempts_per_trajectory = 100;
% The above limit applies to each complete parent trajectory.
cfg.second_level_seed_filter.tolerance = 1e-8;
cfg.second_level_seed_filter.final_track_point_filter_enabled = true;
cfg.second_level_seed_filter.final_track_point_tolerance = 1e-8;
cfg.second_level_seed_filter.final_obstacle_point_filter_enabled = true;
...
cfg.second_level_seed_filter.parallel_trajectory_batch_size = ...
    cfg.second_level_seed_filter.target_trajectory_count;
% Each third-level segment has its own seed; retry the entire failed parent.
% Keep the existing filter switch: this behavior applies when enabled.
cfg.third_level_seed_filter.enabled = 1;
cfg.third_level_seed_filter.implementation_version = 6;
cfg.third_level_seed_filter.seed_scope = 'segment';
cfg.third_level_seed_filter.retry_scope = 'trajectory';
cfg.third_level_seed_filter.base_seed = cfg.third_level_rollout_seed;
cfg.third_level_seed_filter.target_trajectory_count = ...
    cfg.first_level_generation_samples;
cfg.third_level_seed_filter.max_attempts_per_trajectory = 100;
% The above limit applies to each complete parent trajectory.
cfg.third_level_seed_filter.tolerance = 1e-8;
cfg.third_level_seed_filter.final_track_point_filter_enabled = true;
% The global implicit track fields approximate the true lateral margin to
% about 1.2e-3.  The active track constraint already shrinks both rails by
% 3e-3, so accepting a residual no smaller than -5e-4 remains inside the
% physical corridor while avoiding endless retries for numerical boundary
% errors of only 1e-4 to 3e-4.
cfg.third_level_seed_filter.final_track_point_tolerance = 5e-4;
cfg.third_level_seed_filter.final_obstacle_point_filter_enabled = true;
...
% Final straight edges P1-P2 through P4-P5 must remain inside the raw track
% and outside physical obstacles. A failed edge rejects the whole parent.
cfg.third_level_seed_filter.final_track_line_filter_enabled = true;
cfg.third_level_seed_filter.final_obstacle_line_filter_enabled = true;
...
% The third-level-only +0.005 variance-clock shift maps physical t=0.995 to this
% theoretical endpoint.  At and beyond tau=1 terminal_variance_ptcbf uses
% the continuous endpoint values hbar=0 and hbar_dot=0, without evaluating
% the open-interval singular denominator.
