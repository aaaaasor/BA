...
if isfield(cfg.variance_constraint, 'second_level_ptzf_use_cubic_blowup')
    segment_variance_constraint.ptzf_use_cubic_blowup = ...
        cfg.variance_constraint.second_level_ptzf_use_cubic_blowup;
end
...
plot_pt_hocbf_relaxation_terms(cfg, segment_rollout_constraint);
...
if ~use_segment_rollout_cache
    ...
    try
        ...
        save(segment_rollout_cache_path, 'segment_rollout_times', ...
            'segment_traj_path_10d', 'rollout_diagnostics', '-v7.3');
    end
...
if isfield(cfg, 'enable_third_level') && cfg.enable_third_level
    ...
    if isfield(cfg.variance_constraint, 'third_level_ptzf_use_cubic_blowup')
        third_segment_variance_constraint.ptzf_use_cubic_blowup = ...
            cfg.variance_constraint.third_level_ptzf_use_cubic_blowup;
    end
...
if ~isempty(uncertainty_values)
    if isfinite(uncertainty_integral_budget)
        plot_cumulative_uncertainty_budget_vs_time(cfg, uncertainty_times, ...
            uncertainty_values, uncertainty_integral_budget, ...
            [uncertainty_level_label, ' HOCBF variance barrier']);
    end
