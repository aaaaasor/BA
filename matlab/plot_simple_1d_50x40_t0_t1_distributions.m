function result = plot_simple_1d_50x40_t0_t1_distributions()
...
out = fullfile(root,'outputs','1d case全局GP训练阈值实验_50x40');
...
sgtitle(tl,'1D global exact GP (50 training pairs x 40 slices): initial and terminal distributions');
set(findall(f,'Type','axes'),'FontName','Times New Roman','FontSize',11);

emf_path=fullfile(out,'Simple1D_GlobalGP_50x40_t0_t1_True_Distributions.emf');
png_path=fullfile(out,'Simple1D_GlobalGP_50x40_t0_t1_True_Distributions_MobilePreview.png');
...
save(fullfile(out,'Simple1D_GlobalGP_50x40_t0_t1_True_Distributions.mat'), ...
    'result','-v7.3');
fprintf('Saved %s\nSaved %s\n',emf_path,png_path);
