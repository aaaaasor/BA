function result = plot_simple_1d_t0_t1_distributions()
...
initial_samples = randn(n_distribution_samples,1);
...
for i=1:n_models
    M=load(S.runs{i}.source_model,'global_gp_model');
    terminal_samples{i}=rollout_terminal(M.global_gp_model,initial_samples, ...
        S.cfg.n_rollout_steps);
...
for i=1:n_models
    plot(ax,x_grid,terminal_kde(:,i),'LineWidth',1.7,'Color',colors(i,:));
end
xlabel(ax,'State x');ylabel(ax,'Probability density');
...
sgtitle(tl,'1D global exact GP: distributions at the initial and terminal times');
set(findall(f,'Type','axes'),'FontName','Times New Roman','FontSize',11);

emf_path=fullfile(out,'Simple1D_GlobalGP_t0_t1_True_Distributions.emf');
png_path=fullfile(out,'Simple1D_GlobalGP_t0_t1_True_Distributions_MobilePreview.png');
...
save(fullfile(out,'Simple1D_GlobalGP_t0_t1_True_Distributions.mat'), ...
    'result','-v7.3');
fprintf('Saved %s\nSaved %s\n',emf_path,png_path);
