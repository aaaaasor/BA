function run_racing_obstacle_ablation_only()
% Racing baseline ablation: change only the three geometric obstacle-CBF
% enable flags. Variance constraints, variance seed filters, track boundary,
% joint soft-min, time grids, models and all other algorithm settings are
...
verify_only_obstacle_disabled(cfg, baseline_cfg, levels);
...
main_text = strrep(main_text, 'cfg = get_config();', ...
    '% cfg supplied by run_racing_obstacle_ablation_only');
...
fprintf('\n===== Racing ablation: ONLY obstacle constraints OFF =====\n');
...
cd(root);
eval(main_text);
fprintf('RACING OBSTACLE-ONLY ABLATION COMPLETE.\n');
end

function verify_only_obstacle_disabled(cfg, baseline_cfg, levels)
for k = 1:numel(levels)
    p = levels{k};
    assert(baseline_cfg.variance_constraint.([p '_obstacle_enabled']));
    assert(~cfg.variance_constraint.([p '_obstacle_enabled']));
    fields = {'track_boundary_enabled', 'hocbf_enabled', ...
        'ptcbf_enabled', 'joint_safety_softmin_enabled'};
