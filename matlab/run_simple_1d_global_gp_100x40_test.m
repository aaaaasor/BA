function sweep = run_simple_1d_global_gp_100x40_test()
...
out = fullfile(root, 'outputs', '1d case全局GP训练阈值实验_100x40');
...
cfg = struct('version', 1, 'n_train', 100, 'n_time_slices', 40, ...
...
% Nested design: preserve every one of the 50x40 independent pairs,
% then append 50 new pairs.  This isolates the effect of additional
% independent training trajectories from a wholesale dataset replacement.
base_out=fullfile(root,'outputs','1d case全局GP训练阈值实验_50x40');
B=load(fullfile(base_out,'Training_Data_and_Seeds.mat'), ...
    'source_points','target_points');
assert(numel(B.source_points)==50 && numel(B.target_points)==50);
rng(cfg.append_seed);
source_points=[B.source_points(:);randn(50,1)];
target_points=[B.target_points(:);sample_target_1d(50)];
...
sigma_l=H.SigmaL; sigma_f=H.SigmaF; sigma_n=H.SigmaN; hp=H.hp;
assert(abs(sigma_n-cfg.manual_noise_std)<1e-12);
assert(abs(sigma_l(2)-cfg.manual_state_length_scale)<1e-12);
hp.reused_from=fullfile(base_out,'Manual_Hyperparameters.mat');
hp.training_pair_comparison='nested 50 pairs plus 50 appended pairs';
