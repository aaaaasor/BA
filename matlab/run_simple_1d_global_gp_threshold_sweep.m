function sweep = run_simple_1d_global_gp_threshold_sweep()
...
thresholds = [0.30, 0.15, 0.10, 0.08];
cfg = struct('version', 7, 'n_train', 25, 'n_time_slices', 40, ...
    ...
    'manual_noise_std', 0.30, 'manual_state_length_scale', 0.45, ...
...
% Optimize one hyperparameter set and share it across every threshold and
% the full-data model, so only the retained training pairs differ.
hp = struct('n_pretrain', min(450, size(X,1)), ...
    'hyperparameter_mat_path', fullfile(out, 'Shared_Hyperparameters.mat'), ...
    'hyperparameter_cache_signature', struct('cfg', cfg, 'X', X, 'Y', Y));
rng(cfg.hyperparameter_seed);
hp = optimize_gp_hyperparameters(x_slices, y_slices, hp, training_times);
sigma_l = hp.length_scale_mat(:,1);
sigma_f = hp.signal_std_vec(1);
sigma_n = hp.noise_std_vec(1);
% Apply the same manual override to every threshold and the all-data model.
% The optimized time length scale and signal scale remain unchanged.
sigma_l(2) = cfg.manual_state_length_scale;
sigma_n = cfg.manual_noise_std;
hp.length_scale_mat(:,1) = sigma_l;
hp.noise_std_vec(1) = sigma_n;
hp.manual_override = struct('enabled',true,'noise_std',sigma_n, ...
    'state_length_scale',sigma_l(2), ...
    'kept_optimized_time_length_scale',sigma_l(1), ...
    'kept_optimized_signal_std',sigma_f);
